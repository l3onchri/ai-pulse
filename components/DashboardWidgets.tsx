...
// --- AGENT CARD (LEFT COL) ---
// Updated layout: using justify-between and flex-1 on the top section to distribute content evenly
export const AgentCard: React.FC = () => (
  <div className="premium-card bg-white rounded-[32px] p-8 shadow-glow-gold flex flex-col items-center text-center border border-white/60 flex-1 relative overflow-hidden w-full h-full justify-between">
    ...
    {/* TOP SECTION: Avatar & Identity - flex-1 pushes this section to fill available space */}
    <div className="flex flex-col items-center justify-center w-full flex-1 gap-6">
        <div className="relative group cursor-pointer">
          {/* 3/4 circle spinner: border-t-transparent makes the top gap */}
    ...
    <div className="w-full py-6">
        <button className="w-full py-5 bg-pulse-dark text-white rounded-2xl font-bold text-sm flex items-center justify-center gap-3 hover:bg-black transition-all duration-300 hover:scale-105 hover:shadow-glow-button shadow-xl shadow-gray-200/50">
          <RefreshCw size={20} className="animate-spin-slow" /> REFRESH FEED
        </button>
    </div>
...
// --- MENU LINKS (LEFT COL) ---
// Just a static height container at bottom
export const MenuLinks: React.FC = () => (
  <div className="premium-card bg-white rounded-[24px] p-3 shadow-soft border border-white/60 flex flex-col justify-center flex-none">
    {['Preferences', 'History', 'Saved'].map((item, idx) => (
      <div key={idx} className="flex items-center justify-between p-4 hover:bg-pulse-yellow/10 rounded-xl cursor-pointer group transition-all duration-300 mb-1 last:mb-0">
        <span className="text-sm font-bold text-gray-500 group-hover:text-pulse-dark group-hover:translate-x-1 transition-transform">{item}</span>
        <ChevronRight size={16} className="text-gray-300 group-hover:text-pulse-dark group-hover:translate-x-1 transition-transform" />
      </div>
    ))}
  </div>
);
...
export const FeaturedCard: React.FC<{ item: NewsItem }> = ({ item }) => (
  <div className="premium-card bg-white rounded-[32px] p-8 shadow-glow-gold h-[320px] flex flex-col justify-between relative overflow-hidden group border border-white/60">
    ...
    <div className="relative z-10 flex justify-between items-start">
      ...
      <div className="w-10 h-10 rounded-full bg-white shadow-soft flex items-center justify-center text-pulse-dark cursor-pointer hover:scale-110 hover:rotate-6 transition-all duration-300 text-pulse-yellow hover:text-pulse-dark">
    ...
    <div className="relative z-10 mt-auto transform transition-transform duration-300 group-hover:-translate-y-1">
      ...
      <div className="flex items-center justify-between pt-5 border-t border-gray-100/50">
        ...
        <span className="text-xs font-bold text-gray-500 bg-white shadow-sm border border-gray-100 px-2 py-1 rounded-md">15m</span>
      </div>
    </div>
  </div>
);
...
export const LatestDropsCard: React.FC<{ items: NewsItem[] }> = ({ items }) => (
  <div className="premium-card bg-white rounded-[32px] p-8 shadow-soft border border-white/60 h-full">
    <div className="flex items-center justify-between mb-8">
      ...
      <div className="flex bg-gray-100 p-1 rounded-xl">
        {['All', 'Tech', 'Biz'].map((tab, idx) => (
          <button 
            key={tab} 
            className={`px-4 py-1.5 rounded-lg text-[10px] font-bold transition-all duration-300 ${idx === 0 ? 'bg-white shadow-sm text-pulse-dark scale-105' : 'text-gray-400 hover:text-gray-600'}`}
          >
            {tab}
          </button>
        ))}
    ...
    <div className="space-y-4">
      {items.map((item, idx) => (
        <div key={item.id} className="flex items-center gap-5 group cursor-pointer hover:bg-pulse-bg p-3 rounded-2xl transition-all duration-300 -mx-3 hover:translate-x-1 hover:shadow-soft border border-transparent hover:border-white">
          <div className="w-14 h-14 rounded-2xl bg-gray-200 overflow-hidden shrink-0 shadow-sm group-hover:shadow-md group-hover:scale-105 transition-all duration-300 relative">
    ...
    <div className="mt-8 text-center border-t border-gray-100 pt-6">
      <button className="text-[10px] font-bold text-gray-400 uppercase tracking-widest hover:text-pulse-dark transition-all hover:tracking-[0.2em]">
        View Archive
      </button>
    </div>
  </div>
          ...
          <div className="flex items-center justify-between">
             <div className="flex items-center gap-2">
                <span 
                  ...
                  style={{ color: item.color || '#9ca3af' }} // Use the color from data
                >
                   {item.id === 'l1' ? 'TechCrunch' : item.id === 'l2' ? 'The Verge' : item.id.includes('l3') ? 'Reuters' : 'Source'}
                </span>
